...
try {
    ...
    try {
        ...
        const { LibraryApi } = await import(
            '@jellyfin/sdk/lib/generated-client/api/library-api.js'
        );
        ...
        console.log(
            'User API methods:',
            Object.getOwnPropertyNames(Object.getPrototypeOf(userApi)).filter(
                m => !m.startsWith('_')
            )
        );
        console.log(
            'Library API methods:',
            Object.getOwnPropertyNames(Object.getPrototypeOf(libraryApi)).filter(
                m => !m.startsWith('_')
            )
        );
        console.log(
            'Items API methods:',
            Object.getOwnPropertyNames(Object.getPrototypeOf(itemsApi)).filter(
                m => !m.startsWith('_')
            )
        );
        console.log(
            'System API methods:',
            Object.getOwnPropertyNames(Object.getPrototypeOf(systemApi)).filter(
                m => !m.startsWith('_')
            )
        );
    } catch (e) {
        ...
        console.log(
            'Available API files:',
            files.filter(f => f.endsWith('-api.js'))
        );
    }
} catch (error) {
