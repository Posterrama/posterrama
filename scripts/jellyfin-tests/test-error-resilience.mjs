...
async function testErrorResilience() {
    ...
    try {
        ...
        console.log(
            `✅ Normal scenario: ${response.status} - ${response.data.length} items fetched\n`
        );
        ...
        if (jellyfinServer) {
            ...
            try {
                ...
                console.log(
                    `✅ With broken Jellyfin: ${response.status} - ${response.data.length} items fetched (should be Plex only)`
                );
                ...
                console.log(
                    `   ❌ API failed completely: ${error.response?.status || error.message}`
                );
        ...
        try {
            ...
            const configBackup = execSync('cat /var/www/posterrama/config.json.backup', {
                encoding: 'utf8',
            });
            execSync(`echo '${configBackup}' > /var/www/posterrama/config.json`);
            execSync('pm2 restart posterrama', { stdio: 'ignore' });
