...
async function testJellyfinPosters() {
    ...
    try {
        ...
        for (let i = 0; i < Math.min(5, mediaItems.length); i++) {
            ...
            console.log(
                `   - "${item.title}": poster=${!!item.posterUrl}, background=${!!item.backgroundUrl}`
            );
        ...
        console.log(
            `   ✅ Items with posters: ${itemsWithPosters}/${Math.min(5, mediaItems.length)}`
        );
        console.log(
            `   ✅ Items with backgrounds: ${itemsWithBackgrounds}/${Math.min(5, mediaItems.length)}\n`
        );
        ...
        if (itemWithPoster) {
            ...
            try {
                ...
                console.log(
                    `   ✅ Image proxy works: ${imageResponse.status}, ${imageResponse.data.length} bytes`
                );
                console.log(`   ✅ Content-Type: ${imageResponse.headers['content-type']}`);
            } catch (imageError) {
                console.log(
                    `   ❌ Image proxy failed: ${imageError.response?.status || imageError.message}`
                );
            }
        } else {
        ...
        const problematicSvg =
            'data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMSIgaGVpZ2h0PSIxIiB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciPjxyZWN0IHdpZHRoPSIxMDAlIiBoZWlnaHQ9IjEwMCUiIGZpbGw9IiMxMTEiLz48L3N2Zz4=';

        try {
            const svgResponse = await axios.get(
                `${baseUrl}/image?url=${encodeURIComponent(problematicSvg)}`,
                {
                    timeout: 5000,
                    responseType: 'arraybuffer',
                }
            ...
            console.log(
                `   ✅ SVG placeholder works: ${svgResponse.status}, ${svgResponse.data.length} bytes`
            );
        } catch (svgError) {
            console.log(
                `   ❌ SVG placeholder failed: ${svgError.response?.status || svgError.message}`
            );
        }
