...
function checkRateLimiter() {
  ...
  expected.forEach(l => {
    console.log(`Expect ${l.name}: windowMs=${l.windowMs} (${l.windowMs/60000}m), max=${l.max}`);
  });
  console.log('Note: Actual values are defined in server.js; this script validates assumptions only.');
}
...
function summarize() {
  section('Result');
  if (errors.length === 0) console.log('Errors: none'); else {
    console.log('Errors:');
    errors.forEach(e => console.log(' - ' + e));
  }
  if (warnings.length === 0) console.log('Warnings: none'); else {
