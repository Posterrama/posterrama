...
function showMedia(index) {
    ...
    if (appConfig.showMetadata !== false) {
        ...
        if (taglineEl) taglineEl.textContent = media.tagline || '';
        if (yearEl) yearEl.textContent = media.year ? `(${media.year})` : '';
    ...
    // Show clear logo if enabled and available
    if (appConfig.showClearLogo && media.clearLogoUrl) {
        const clearlogoEl = document.getElementById('clearlogo');
        const clearLogoEl = document.getElementById('clear-logo');
        if (clearlogoEl) {
            clearlogoEl.style.backgroundImage = `url('${media.clearLogoUrl}')`;
            clearlogoEl.style.display = 'block';
            clearlogoEl.style.opacity = '0.8';
        }
        if (clearLogoEl) {
            clearLogoEl.style.backgroundImage = `url('${media.clearLogoUrl}')`;
            clearLogoEl.style.display = 'block';
