...
function restartPM2ForEnvUpdate(reason = 'environment update', async = true) {
    ...
    if (async) {
        ...
        // Awaitable execution (do not block the event loop)
 ...
 * @typedef {Object} ServerRequestBody
 * @property {string} [hostname] - Optional hostname from request
 * @property {string} [host] - Alternative hostname property
 * @property {number} [port] - Optional port from request
 * @property {string} [token] - Optional token from request
 * @property {string} [apiKey] - Alternative token property
 */
 ...
 * @param {ServerRequestBody} options.requestBody - Request body from Express req.body
 * @returns {{hostname: string, port: number, token: string, serverConfig: Object}} Server configuration with resolved values
...
function getServerConfig({ config, serverType, requestBody = {} }) {
    // Find server configuration (optional - can use request body only)
    const serverConfig = config.mediaServers?.find(s => s.type === serverType);

    // Extract and sanitize hostname from request body first, then config
    ...
    // Extract port from request body first, then config
    let port = requestBody.port;
    if (!port && typeof serverConfig?.port !== 'undefined') {
        port = serverConfig.port;
    }

    // Extract token/apiKey from request body first, then environment variable
