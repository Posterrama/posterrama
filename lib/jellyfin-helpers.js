...
async function enrichJellyfinItemWithExtras(item, serverConfig, client = null) {
    ...
    // Extract the Jellyfin ID from the composite key (format: "jellyfin_ServerName_abc123")
    const keyParts = item.key.split('_');
    if (keyParts.length < 3 || keyParts[0] !== 'jellyfin') {
        return item;
    }

    const itemId = keyParts[keyParts.length - 1]; // Last part is the Jellyfin ID
    ...
    try {
        ...
        // Fetch special features (trailers, behind the scenes, etc.)
        let extras = null;
        try {
            ...
            // Silently ignore - not all items have extras
        }

        // Find first trailer for convenience
        const trailer = extras?.find(e => e.type === 'trailer') || null;

        // Note: Jellyfin does not have theme music URLs in the same way as Plex
        // Theme music would need to be handled through regular audio playback
