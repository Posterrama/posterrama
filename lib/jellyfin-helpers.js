...
async function processJellyfinItem(item, serverConfig, client) {
    try {
        ...
        // Extract metadata
        // NOTE: Prefer a server-qualified key when available so that selected-item generation
        // can reliably route back to the correct Jellyfin/Emby server in multi-server setups.
        const jellyfinKey = serverConfig?.name
            ? `jellyfin_${serverConfig.name}_${item.Id}`
            : `jellyfin_${item.Id}`;

        const processedItem = {
            id: `jellyfin_${item.Id}`,
            key: jellyfinKey, // Add key property for consistency with Plex
