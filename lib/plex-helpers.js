...
async function getPlexMusicGenres(serverConfig, libraryKey) {
    ...
    try {
        ...
        // Debug: Log first genre to see structure
        if (genres.length > 0) {
            logger.info('[Plex Music Genres] First genre from API:', JSON.stringify(genres[0]));
            logger.info('[Plex Music Genres] All fields:', Object.keys(genres[0]));
        }

        // Map to simple format and sort by count descending
...
async function getPlexMusicArtists(serverConfig, libraryKey, limit = 100, offset = 0) {
    ...
    try {
        ...
        // Debug: Log first artist to see structure
        if (artistItems.length > 0) {
            logger.info(
                '[Plex Music Artists] First artist from API:',
                JSON.stringify(artistItems[0])
            );
            logger.info('[Plex Music Artists] All fields:', Object.keys(artistItems[0]));
        }
