...
class BaseHttpClient {
    ...
    /**
     * Mask URL to hide sensitive information while keeping protocol and domain visible
     * @param {string} url - URL to mask
     * @returns {string} Masked URL (e.g., "https://example.com:443")
     * @private
     */
    _maskUrl(url) {
        try {
            const parsed = new URL(url);
            // Show protocol, hostname, and port only (hide path, query, credentials)
            return `${parsed.protocol}//${parsed.hostname}${parsed.port ? ':' + parsed.port : ''}`;
        } catch {
            // If URL parsing fails, show only first 10 chars + "..."
            return url.length > 10 ? url.slice(0, 10) + '...' : '[invalid-url]';
        }
    }
