...
async function getPlaylistMedia({
    ...
    const fetchFromServer = async server => {
        ...
        try {
            ...
            if (server.type === 'romm') {
                ...
                return { server, media: [], lastFetch: null };
            }
            ...
            return { server, media: mediaFromServer, lastFetch };
        } catch (error) {
            // Server completely failed - log but return empty result
    ...
    for (const result of serverResults) {
        if (result.status === 'fulfilled' && result.value) {
            const { server, media, lastFetch } = result.value;
            allMedia = allMedia.concat(media);

            // Update lastFetch tracking
            if (lastFetch) {
                if (server.type === 'plex') {
                    latestLastFetch.plex = Math.max(latestLastFetch.plex || 0, lastFetch);
                } else if (server.type === 'jellyfin') {
                    latestLastFetch.jellyfin = Math.max(latestLastFetch.jellyfin || 0, lastFetch);
                } else if (server.type === 'romm') {
                    latestLastFetch.romm = Math.max(latestLastFetch.romm || 0, lastFetch);
                }
            }
        } else if (result.status === 'rejected') {
            logger.error('Media server fetch promise rejected:', {
    ...
    const fetchFromTMDB = async () => {
        ...
        try {
            ...
            return { media: tmdbMedia, lastFetch };
        } catch (error) {
    ...
    const fetchFromLocal = async () => {
        ...
        try {
            ...
            return { media: normalized, lastFetch };
        } catch (error) {
    ...
    if (tmdbResult.status === 'fulfilled' && tmdbResult.value) {
        ...
        if (tmdbResult.value.lastFetch) {
            latestLastFetch.tmdb = Math.max(latestLastFetch.tmdb || 0, tmdbResult.value.lastFetch);
        }
    }
    ...
    if (localResult.status === 'fulfilled' && localResult.value) {
        ...
        if (localResult.value.lastFetch) {
            latestLastFetch.local = Math.max(
                latestLastFetch.local || 0,
                localResult.value.lastFetch
            );
        }
    }
    ...
    logger.info(`[Parallel Fetch] All sources completed in ${totalFetchTime}ms`, {
