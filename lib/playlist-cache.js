...
async function refreshPlaylistCache({ getPlaylistMediaWrapper, shuffleArray, totalCap }) {
    if (isRefreshing) {
        // Check if the current refresh is stuck (must be > timeout duration to avoid race)
        // Timeout is 45s, so we check for stuck state at 60s
    ...
    const refreshTimeout = setTimeout(() => {
        ...
        isRefreshing = false;
        refreshStartTime = null;
    }, 45000); // 45 second timeout (allows for slow Plex queries)
