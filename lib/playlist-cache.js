...
async function refreshPlaylistCache({ getPlaylistMediaWrapper, shuffleArray, totalCap }) {
    // Deduplication: return existing promise if refresh in progress
    // CRITICAL: Check refreshPromise first to avoid race conditions
    if (refreshPromise) {
        logger.debug('Playlist refresh already in progress - returning existing promise');
        return refreshPromise;
    }

    // Set flag and create promise atomically
    isRefreshing = true;
    ...
    refreshPromise = (async () => {
        try {
            ...
            if (duration > 25000) {
                // 25 seconds threshold - realistic for large libraries with multiple servers
            ...
            refreshPromise = null;
            logger.debug('[PlaylistCache] Refresh state cleared', {
                cacheItems: playlistCache ? playlistCache.length : 0,
            });
        }
    })();
...
function isPlaylistRefreshing() {
    // If refresh flag is set but cache exists and is recent (< 5min old), clear stuck state
    if (isRefreshing && playlistCache && playlistCache.length > 0 && cacheTimestamp) {
        const ageMs = Date.now() - cacheTimestamp;
        if (ageMs < 5 * 60 * 1000) {
            // Cache is fresh - refresh must have completed, clear stuck flag
            logger.warn('[PlaylistCache] Detected stuck refresh flag with fresh cache - clearing', {
                cacheAge: `${Math.round(ageMs / 1000)}s`,
                cacheItems: playlistCache.length,
            });
            isRefreshing = false;
            refreshStartTime = null;
            refreshPromise = null;
    ...
    // Timeout-based stuck detection: if refreshing for more than 5 minutes, clear stuck state
    if (isRefreshing && refreshStartTime) {
        const refreshDuration = Date.now() - refreshStartTime;
        if (refreshDuration > 5 * 60 * 1000) {
            // 5 minutes timeout
            logger.error(
                '[PlaylistCache] Refresh stuck for over 5 minutes - clearing stuck state',
                {
                    action: 'playlist_refresh_timeout',
                    duration: `${Math.round(refreshDuration / 1000)}s`,
                    cacheItems: playlistCache ? playlistCache.length : 0,
                }
            );
            isRefreshing = false;
