...
// Only define tests if broker is configured
if (MQTT_BROKER) {
    describe('MQTT Integration E2E (Real Broker)', () => {
        let mqttBridge;
        let testClient;
        ...
        // Close main E2E describe block
    });
    // End of MQTT E2E tests - if block
} else {
    // When MQTT broker is not configured, create a passing test to document why
    describe('MQTT Integration E2E (Not Configured)', () => {
        test('MQTT broker not configured - E2E tests not run', () => {
            // This is informational only - E2E tests require MQTT_TEST_BROKER env var
            // To run E2E tests: export MQTT_TEST_BROKER="mqtt://your-broker:1883"
            expect(MQTT_BROKER).toBeUndefined();
        });
    });
}
