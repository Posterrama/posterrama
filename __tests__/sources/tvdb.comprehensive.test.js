...
describe('TVDBSource comprehensive', () => {
    ...
    describe('authentication & request retry', () => {
        ...
        test('authenticate caches token until expiry', async () => {
            ...
            await src.authenticate();
            expect(axios.post).toHaveBeenCalledTimes(firstCalls);
            src.tokenExpiry = Date.now() - 1000;
            axios.post.mockResolvedValueOnce({ data: { data: { token: 'tokB' } } });
            await src.authenticate();
            expect(axios.post).toHaveBeenCalledTimes(firstCalls + 1);
        });
            ...
            queueGet([
                {
                    data: {
                        artworks: [
                            { type: 5, image: '/fan.jpg' },
                            { type: 2, language: 'eng', image: '/post.jpg' },
                        ],
                    },
                },
            ]);
    ...
    describe('shows & movies fetching', () => {
        test('getShows applies rating/year/genre filters and caches', async () => {
            ...
            src.getArtwork = jest.fn().mockResolvedValue({
                fanart: 'https://artworks.thetvdb.com/fan1.jpg',
                poster: 'https://artworks.thetvdb.com/p1.jpg',
            });
            ...
            const c = axios.get.mock.calls.length;
            await src.getShows();
            expect(axios.get).toHaveBeenCalledTimes(c);
        });
        ...
        test('getMovies processes and limits movie count with artwork', async () => {
            ...
            src.getArtwork = jest
                ...
                .mockResolvedValueOnce({
                    fanart: 'https://artworks.thetvdb.com/fanM1.jpg',
                    poster: 'https://artworks.thetvdb.com/pM1.jpg',
                })
    ...
    describe('utilities', () => {
        ...
        test('cache stats and clearCache', () => {
