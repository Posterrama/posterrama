...
describe('Plex Source', () => {
    ...
    describe('fetchMedia', () => {
        ...
        it('should respect rtMinScore filtering', async () => {
            ...
            const mockMediaItems = [
                ...
                { ratingKey: '2', title: 'Bad Movie', rating: 5.0 },
            ...
            mockProcessPlexItem
                .mockResolvedValueOnce({
                    title: 'Good Movie',
                    rottenTomatoes: { originalScore: 0.85 },
                })
                .mockResolvedValueOnce({
                    title: 'Bad Movie',
                    rottenTomatoes: { originalScore: 0.5 },
                })
                .mockResolvedValueOnce({
                    title: 'Great Movie',
                    rottenTomatoes: { originalScore: 0.9 },
                });
        ...
        it('should log debug information when debug is enabled', async () => {
            ...
            // Since we moved to logger.info, we check the debug property instead
            const result = await debugPlexSource.fetchMedia(['Movies'], 'movie', 5);
            expect(debugPlexSource.isDebug).toBe(true);
    ...
    describe('Content Filtering', () => {
        it('should filter by rating', async () => {
            const serverConfigWithRating = { ...mockServerConfig, ratingFilter: 'PG-13' };
            const plexSourceWithRating = new PlexSource(
                serverConfigWithRating,
                mockGetPlexClient,
                mockProcessPlexItem,
                mockGetPlexLibraries,
                mockShuffleArray,
        ...
        it('should filter by genre', async () => {
            const serverConfigWithGenre = { ...mockServerConfig, genreFilter: 'Action, Comedy' };
            ...
            const mockItems = [
                ...
                { ratingKey: '2', title: 'Drama Movie', Genre: [{ tag: 'Drama' }] },
                {
                    ratingKey: '3',
                    title: 'Comedy Movie',
        ...
        it('should filter by quality', async () => {
            const serverConfigWithQuality = { ...mockServerConfig, qualityFilter: '1080p' };
            const plexSourceWithQuality = new PlexSource(
                serverConfigWithQuality,
                mockGetPlexClient,
                mockProcessPlexItem,
                mockGetPlexLibraries,
                mockShuffleArray,
            ...
            const mockItems = [
                { ratingKey: '1', title: 'HD Movie', Media: [{ videoResolution: '1080' }] },
                { ratingKey: '2', title: 'SD Movie', Media: [{ videoResolution: 'sd' }] },
                { ratingKey: '3', title: 'Another HD Movie', Media: [{ videoResolution: '1080' }] },
            ];
        ...
        it('should track and report performance metrics', () => {
            ...
            expect(metrics).toHaveProperty('itemsFiltered');
            expect(metrics).toHaveProperty('filterEfficiency');
            expect(typeof metrics.filterEfficiency).toBe('number');

            // Test metrics reset
            plexSource.resetMetrics();
            const resetMetrics = plexSource.getMetrics();
            expect(resetMetrics.requestCount).toBe(0);
            expect(resetMetrics.itemsProcessed).toBe(0);
        });
    });
});
