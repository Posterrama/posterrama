...
describe('TMDBSource', () => {
	...
	describe('getEndpoint', () => {
		...
		test('handles discover movie', () => {
			...
			expect(source.getEndpoint('movie', 3)).toBe('/discover/movie?page=3&sort_by=popularity.desc');
		});
	...
	describe('formatStreamingProviders', () => {
		test('returns unavailable when no data', () => {
			expect(source.formatStreamingProviders(null)).toEqual({ available: false, providers: [] });
		});
	...
	describe('processTMDBItem', () => {
		test('processes movie item without streaming', async () => {
			const genreMap = new Map([[1, 'Action']]);
			const item = { id: 10, title: 'Test', backdrop_path: '/b.jpg', poster_path: '/p.jpg', overview: 'Desc', vote_average: 5, release_date: '2024-05-05', genre_ids: [1] };
		...
		test('adds streaming data when requested', async () => {
			global.fetch = createFetchMock([
				{ json: { results: { US: { flatrate: [{ provider_name: 'Netflix', logo_path: '/n.png' }] } } } }
			]);
	...
	describe('cleanupCache & getCacheStats', () => {
		test('removes expired entries', async () => {
			// Seed caches
			source.genreCache.set('genres_movie', { data: new Map(), timestamp: Date.now() - source.cacheTTL - 1000 });
			source.responseCache.set('url1', { data: { a: 1 }, timestamp: Date.now() - source.cacheTTL - 1000 });
	...
	describe('fetchMedia integration (simplified)', () => {
		test('fetches popular movies with filtering and processing', async () => {
			...
			global.fetch = createFetchMock([
				...
				{ json: { results: [
					{ id: 1, title: 'A', vote_average: 8, release_date: '2024-01-01', genre_ids: [1] },
					{ id: 2, title: 'B', vote_average: 7, release_date: '2024-01-01', genre_ids: [1] }
				] } }
			]);
		...
		test('handles latest category single object', async () => {
			...
			global.fetch = createFetchMock([
				{ json: { genres: [{ id: 1, name: 'Action' }] } }, // genre
				{ json: { id: 99, title: 'Latest Movie', vote_average: 5, release_date: '2024-02-02', genre_ids: [1] } }
			]);
		...
		test('continues on page fetch errors', async () => {
			...
			global.fetch = createFetchMock([
				...
				{ json: { results: [{ id: 1, title: 'X', vote_average: 9, release_date: '2024-01-01', genre_ids: [] }] } } // page 2 success
			]);
...
describe('TMDBSource edge cases (merged)', () => {
	const base = { name: 'tmdb1', apiKey: 'K', category: 'streaming_new_releases', watchRegion: 'NL', minRating: 0 };
	const identity = a => a;
	let source;
	const realFetch = global.fetch;
	function buildFetch(sequence) { let i=0; return jest.fn(async ()=>{ const def = sequence[Math.min(i, sequence.length-1)]; i++; if(def.error) throw def.error; return { ok: def.ok !== false, status: def.status||200, statusText: def.statusText||'OK', json: async ()=> def.json || {} }; }); }
	afterAll(()=>{ global.fetch = realFetch; });
	beforeEach(()=>{ source = new (require('../../sources/tmdb'))({...base}, identity, true); source.minRequestInterval=0; source.retryDelay=1; });

	test('fetchStreamingProviders caches and handles missing region data', async () => {
		global.fetch = buildFetch([
			{ json: { results: { US: { flatrate: [{ provider_name: 'Prov', logo_path: '/p.png' }], link: 'x' } } } },
			{ json: { results: { US: { flatrate: [{ provider_name: 'Prov2', logo_path: '/p2.png' }] } } } }
		]);
		const d1 = await source.fetchStreamingProviders('movie', 1);
		expect(d1.results.US.flatrate.length).toBe(1);
		const d2 = await source.fetchStreamingProviders('movie', 1);
		expect(global.fetch).toHaveBeenCalledTimes(1);
	...
	test('applyContentFiltering returns empty when genre_ids missing for genre filter', () => {
		source.source.genreFilter = 'Action';
		const genreMap = new Map([[1,'Action']]);
		const items = [{ id:1, vote_average:9 }, { id:2, vote_average:8, genre_ids: [2] }];
		const filtered = source.applyContentFiltering(items, 'movie', genreMap);
	...
	test('cleanupCache removes expired streaming provider cache entries', () => {
		...
		source.responseCache.set(key, { data: { a:1 }, timestamp: Date.now() - source.cacheTTL - 10 });
		source.cleanupCache();
