...
const testLogger = ['debug', 'info', 'warn', 'error'].reduce((acc, l) => {
    acc[l] = (...a) => logCalls[l].push(a);
    return acc;
}, {});
...
describe('CacheManager core operations', () => {
    ...
    test('persistence write failure logs warning without crashing', async () => {
        ...
        logCalls.warn.some(
            args =>
                String(args[0] || '').includes('Failed to persist') ||
                String(args[0] || '').includes('persist')
        );
...
describe('cacheMiddleware integration', () => {
    ...
    test('first request MISS then HIT with ETag handling', async () => {
        ...
        const body =
            typeof second.body === 'object' && Object.keys(second.body).length
                ? second.body
