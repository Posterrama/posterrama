describe('GitHub Service - Error Handling', () => {
    ...
    describe('makeRequest error scenarios', () => {
        test('should handle makeRequest errors by mocking the method directly', async () => {
            ...
            GitHubService.makeRequest = jest.fn().mockRejectedValue(new Error('GitHub API error: 404 - Not Found'));
            
            await expect(GitHubService.getLatestRelease()).rejects.toThrow('GitHub API error: 404 - Not Found');
        ...
        test('should handle JSON parse errors by mocking makeRequest', async () => {
            const originalMakeRequest = GitHubService.makeRequest;
            GitHubService.makeRequest = jest.fn().mockRejectedValue(new Error('Failed to parse GitHub API response'));
            
            await expect(GitHubService.getRepositoryInfo()).rejects.toThrow('Failed to parse GitHub API response');
        ...
        test('should handle network errors by mocking makeRequest', async () => {
            const originalMakeRequest = GitHubService.makeRequest;
            GitHubService.makeRequest = jest.fn().mockRejectedValue(new Error('GitHub API request failed: Network error'));
            
            await expect(GitHubService.getReleases()).rejects.toThrow('GitHub API request failed: Network error');
        ...
        test('should handle timeout errors by mocking makeRequest', async () => {
            const originalMakeRequest = GitHubService.makeRequest;
            GitHubService.makeRequest = jest.fn().mockRejectedValue(new Error('GitHub API request timeout'));
            
            await expect(GitHubService.checkForUpdates()).rejects.toThrow('GitHub API request timeout');
        ...
        test('should handle various API error codes', async () => {
            ...
            const errorScenarios = [
                ...
                { error: 'GitHub API error: 429 - Rate limit exceeded', method: 'getRepositoryInfo' }
            ];
