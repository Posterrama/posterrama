...
**Total Scripts:** 28 top-level scripts + 9 validation scripts (plus libraries under `scripts/lib/`)

### By Category

| Category                 | Purpose                                 |
| ------------------------ | --------------------------------------- |
| **Testing & Validation** | Test execution, API validation, health  |
| **Configuration**        | Config validation and verification      |
| **Maintenance**          | MQTT, device management, runtime tasks  |
| **Build & Deploy**       | Production deployment, icons, secrets   |
| **Documentation**        | OpenAPI, coverage tables, release notes |
| **Quality**              | Security audit, git hooks               |
...
**Purpose:** Update README badges (version; optionally coverage if present)
...
**Notes:** Coverage badge updates are skipped unless a coverage badge is present in README.
...
3. `restart-with-env.sh` - PM2 restart with fresh env
...
node scripts/mqtt-cleanup-entities.js # Remove old/orphaned HA MQTT entities
node scripts/mqtt-republish-discovery.js # Force republish MQTT discovery
```
